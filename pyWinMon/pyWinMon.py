...
def data():
    while True:
        cpu_p = psutil.cpu_percent(interval=1, percpu=True)  # 读取每核CPU使用率
        t3.set(cpu_p)
        cpu_t = psutil.cpu_percent(1)  # 读取总CPU使用率
        t7.set(cpu_t)
...
if __name__ == '__main__':
    ...
    master.geometry("350x150+100+100")  # 窗口呈现大小和位置
    ...
    Label(master, text="总CPU使用率(%)：").grid(row=1, column=0, columnspan=4)
    Label(master, textvariable=t7).grid(row=1, column=4)
    Label(master, text="单核CPU使用率(%)：").grid(row=2, column=0, columnspan=4)
    Label(master, textvariable=t3).grid(row=2, column=4)
    Label(master, text="内存使用率：").grid(row=3, column=0, columnspan=4)
    Label(master, textvariable=t4).grid(row=3, column=4)
    Label(master, text="网络(下/上)：").grid(row=4, column=0, columnspan=4)
    Label(master, textvariable=t5).grid(row=4, column=4)
    Label(master, text="硬盘使用率：").grid(row=5, column=0, columnspan=4)
    Label(master, textvariable=t6).grid(row=5, column=4)
